...
import { AnimatedCard } from "@/components/chat/animated-card";
...
export function ContactInfo({ onSubmit, status }: ContactInfoProps) {
  ...
  const inputProps = (value: string, setter: (value: string) => void) => ({
    value,
    className: "border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:outline-none rounded-md p-2 disabled:bg-white disabled:cursor-not-allowed disabled:text-gray-500",
  ...
  return (
    <AnimatedCard status={status}>
      ...
      <h2 className="text-center text-base text-gray-400 antialiased">We need this information in order to process your order and contact you if there are any issues.</h2>
